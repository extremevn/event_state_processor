...
class HomeEventProcessor extends EventToStateProcessor<HomeEvent, HomeDataState> {
  @override
  HomeDataState get initialState => const HomeDataState(isInit: true, isLoading: false, pokemons: [], currentPage: 0);
  ...
  Stream<HomeDataState> processEvent(HomeEvent event) async* {
    if (event is LoadDataEvent) {
      try {
        ...
        yield state.copy(isLoading: false, dataPokemon: data, currentPage: event.page);
      } catch (error) {
